...
def main():
    path = input("Введите путь к директории с плейлистами: ")
    if not Path(path).exists() or not Path(path).is_dir() or path == "" or path == " ":
        print("Введенная директория не существует")
        sys.exit(0)
