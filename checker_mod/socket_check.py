...
class SockCheck:
    ...
    def get_ip(self, url, ext):
        host = urlparse(url).hostname
        try:
            return socket.gethostbyname(str(host))
        except Exception:
            self.ip_error.append(f'{ext}\n{url}\n')
            self.print_status()
            self.print_status()
            return False
    ...
    def print_status(self):
        ...
        print(f"\r{Fore.YELLOW}Sock Check: {Fore.RESET}"
              f"{len(self.good_sock) + len(self.bad_sock) + len(self.ip_error)}/"
              f"{len(self.exts)} | {Fore.YELLOW}Good Sock: {Fore.RESET}{len(self.good_sock)} | "
              f"{Fore.YELLOW}Bad Sock: {Fore.RESET}{len(self.bad_sock) + len(self.ip_error)}", end="")

    def sock_connect(self, port, ext, url):
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.settimeout(7)
        try:
            if ip := self.get_ip(url, ext):
                s.connect((ip, port))
    ...
    def sock_scan(self):
        ...
        print(f"\n{Fore.CYAN}Sock Online: {Fore.RESET}{len(self.good_sock)} | {Fore.CYAN}Sock Offline: "
              f"{Fore.RESET}{len(self.bad_sock) + len(self.ip_error)}")
        print(Fore.GREEN + "-" * 60 + "\n")
        return self.good_sock, self.bad_sock, self.ip_error
