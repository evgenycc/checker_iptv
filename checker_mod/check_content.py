...
stop_list = ["/errors/", "test_end.ts", "buy.ts", "money.ts", "buy_packet.ts", "empty.ts", "key.ts", "/error/",
             ...
             "auth", "logout", "VDO-X-404", "offline", "logout.mp4", "error.tv4.live", "block-ip-video.ts", "/407/",
...
class ContentCheck:
    ...
    def find_stop(url):
        if "route441cz" in str(urlparse(url).hostname):
            if 'test_end.ts' in str(urlparse(url).path):
                return True
            return False
        for st in stop_list:
            if findall(f"{st}", url):
    ...
    def print_status(self):
        print("\r\033[K", end="")
        print(f"\r{Fore.YELLOW}Check Content: {Fore.RESET}{len(self.online) + len(self.offline)}/{len(self.good_status)} | "
    ...
    def check_content(self, url, ext):
        timeout = 15 if self.re_ch else 35
            ...
            with httpx.stream("GET", url=url, headers=headers, timeout=timeout, follow_redirects=True, verify=False) as res:
                if 200 <= res.status_code <= 299:
                    if ".mpd" in str(res.url):
                        return True if "xml" in res.read() else False
            ...
            for item in self.good_status:
                ...
                if len(temp) >= 5:
                    for tm in temp:
                        ext = tm.split("\n")[0].strip()
                        url = tm.split("\n")[1].strip()
                        executor.submit(self.verify, url=url, ext=ext)
                    temp.clear()
            if len(temp) < 5:
                for tm in temp:
                    ext = tm.split("\n")[0].strip()
